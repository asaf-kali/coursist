...
class Course(Base):
    ...
    def find_by(text: str, school: str, faculty:str):
        return Course.objects.filter((Q(name__contains=text) | Q(course_number__icontains=text)) & Q(
            school__name__icontains=school) & Q(school__faculty__name__icontains=faculty)).all()
    ...
    @staticmethod
    def find_by_school(text: str):
        return Course.objects.filter(Q(school__name__icontains=text)).all()

    @staticmethod
    def find_by_faculty(text: str):
        return Course.objects.filter(Q(school__faculty__name__contains=text)).all()